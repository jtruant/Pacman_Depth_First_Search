...
class CornersProblem(search.SearchProblem):
  ...
  def getStartState(self):
    ...
    "*** YOUR CODE HERE ***"
    """
    N=copy.deepcopy(self.North)
    E=copy.deepcopy(self.East)
    S=copy.deepcopy(self.South)
    W=copy.deepcopy(self.West)
    """
    #actual vector for starting position
    startVector=self.startingPosition
    #return the starting vector as well as representations of 
    #untouched corners
    actualStart=((startVector),['N','E','S','W'])
    return actualStart
  ...
  def isGoalState(self, state):
    ...
    "*** YOUR CODE HERE ***"
    "if the state is one of the corners,"
    
    if(state[0]==self.corners[0]):
    #remove north from state
     for i in state[1]:
     ...
     #remove east from state
     for i in state[1]:
      if i=='E':
       state[1].remove('E')
       
    elif(state[0]==self.corners[2]):
     #remove south from state
     for i in state[1]:
      if i=='S':
       state[1].remove('S')
       
    elif(state[0]==self.corners[3]):
     #remove west from state
     for i in state[1]:
      if i=='W':
       state[1].remove('W')
       
       #do something here
    "if the length of unreached corners is zero, all corners have been reached"
    if((len(state[1]))==0):
       isGoal=True
    else:
       isGoal=False
    ...
    if(state[0]==self.corners[0]):
       ...
       for i in state[1]:
        ...
        if i=='N':
         ...
         state[1].remove('N')
         print "at North corner", state[0], state[1]
...
def cornersHeuristic(state, problem):
  ...
  #stringState = str(xy1[0])
  #xy1 = list(xy1[0])
  #print "xy1[0]", xy1[0]
  

  lowest = 1000000000000
  #lowest = abs(xy[0] - corner1[0]) + abs(xy[1] - corner1[1])
  
 
  for i in corners:
    i = list(i)
    stateCoor = list(state[0])
    if (stateCoor != i):
...
def foodHeuristic(state, problem):
  ...
  If using A* ever finds a solution that is worse then uniform cost search finds,
